...
class SimpleEmbeddingService:
    ...
    def _load_model(self):
        ...
        # Skip sentence transformer model loading due to network issues
        # Use hash-based fallback instead
        logger.info("Using hash-based embedding fallback to avoid network dependencies")
        self.model = None
    
    def encode(self, text: str) -> List[float]:
        """Encode text using sentence transformer or hash-based fallback"""
        if not text.strip():
            return []
        ...
        if self.model:
            try:
                ...
                logger.warning(f"Failed to encode with model: {e}, using fallback")
    ...
    def query_similar(self, query_text: str, n_results: int = 5, threshold: float = 0.3):
        ...
        for doc_id, chunks in self.documents.items():
            ...
            for i, chunk_embedding in enumerate(doc_embeddings):
                ...
                # Always add results regardless of similarity for hash-based embeddings
...
async def rag_query(request: dict):
    """RAG query endpoint that matches frontend expectations"""
    global current_document_id
    ...
    try:
        ...
        if not current_document_id or current_document_id not in vector_db.documents:
            fallback_response = (
                f"I don't have access to any documents to answer your question about '{query}'. "
                f"Please upload a document first, and I'll be happy to help you find information."
            )
            processing_time = (time.time() - start_time_ms) * 1000
        ...
        # Perform similarity search
        logger.info(f"Querying for: '{query}' with {len(vector_db.documents)} documents in DB")
        search_results = vector_db.query_similar(
            query_text=query,
            n_results=5,
            threshold=config.similarity_threshold
        ...
        if not search_results:
            fallback_response = (
                f"I couldn't find relevant information about '{query}' "
                f"in the uploaded documents. Please try rephrasing your question or "
                f"uploading relevant documents."
            )
            processing_time = (time.time() - start_time_ms) * 1000
        ...
        # Generate response using Groq API or fallback
        context_text = "\n\n".join([result.content for result in search_results])
                ...
                with open("env.txt", "r") as f:
                    for line in f:
                        if line.startswith("GROQ_API_KEY="):
                            ...
                            groq_response = call_groq_api_for_rag(query, context_text, api_key)
                            break
            except Exception as e:
                logger.warning(f"Failed to use Groq API: {e}")
        ...
        if groq_response:
            ...
            # Fallback response
            answer = (
                f"Based on the uploaded document, here's what I found about '{query}':\n\n"
                f"{context_text[:500]}..."
            )
        ...
        return {
            ...
            "answer": answer,
            "confidence_score": confidence_score,
            "sources": [{"text": result.content, "score": result.similarity_score} for result in search_results],
            "processing_time_ms": processing_time,
            "fallback_used": groq_response is None
        }
...
def call_groq_api_for_rag(query: str, context: str, api_key: str) -> str:
