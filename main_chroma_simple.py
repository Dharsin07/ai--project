...
class SimpleVectorStore:
    ...
    def add(self, documents: List[str], embeddings: np.ndarray, metadatas: List[Dict[str, Any]], ids: List[str]):
...
def generate_mock_embeddings(texts: List[str]) -> np.ndarray:
    """Generate mock embeddings when Sentence Transformers is not available"""
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    try:
        ...
        vector_store.add(all_chunks, embeddings, chunk_metadata, chunk_ids)
...
async def health_check():
    ...
    return {
        ...
        "embedding_model": {
            ...
            "model_name": "all-MiniLM-L6-v2" if embedding_model else "Mock Embeddings"
        },
...
async def root():
    ...
    return {
        ...
        "version": "2.0.0",
        "vector_store": "Simple Vector Store (ChromaDB-like)",
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting AI Research Assistant API with Simple Vector Store")
